...
Allow configuring a webhook in Stripe to send s2s notifications to Eagle
...
is redirected to Eagle (e.g. network loss, browser crash, closing the
tab, etc.).

The webhook should be configured to send its events to
<base_url>/payment/stripe/webhook and should only subscribe to
checkout.session.completed events to avoid spamming the Eagle server with
