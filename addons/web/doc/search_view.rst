...
EagleERP Web 7.0 implements a unified facets-based search view instead
...
makes significant use of Backbone's models and collections (EagleERP
Web's widgets make a good replacement for Backbone's own views). As a
result, understanding the implementation details of the EagleERP Web 7
...
The primary component of search views, as with all other EagleERP
views, are inputs. The search view has two types of inputs — filters
and fields — but only one is easly customizable: fields.

The mapping from EagleERP field types (and widgets) to search view
...
EagleERP this is done via :ref:`domains <openerpserver:domains>`. On
the other hand, the EagleERP Web 7 search view's state is modelled
...
:js:class:`~openerp.web.search.Facet` objects to EagleERP search data.
...
of the peculiarities pertaining to EagleERP's handling of fields in
...
EagleERP Web 6.1 to EagleERP Web 7.
