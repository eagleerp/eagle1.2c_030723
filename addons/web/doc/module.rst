...
A very basic EagleERP module structure will be our starting point:
...
There is no such thing as a "web module" declaration. An EagleERP
...
.. note::
    ...
    restart the EagleERP server itself for it to be taken in account.
...
provides: an instance of the EagleERP Web client, with the contents of
...
will require restarting the EagleERP Server to see the changes, as
usual):
...
.. note::
    ...
    will apply only within the confines of the EagleERP Web client) and
...
EagleERP Web's template language is :doc:`qweb`. Although any
...
integration to EagleERP Web widgets.
...
To prevent this irremediable loss, we can use EagleERP's support for
...
basic EagleERP model in which our data will be stored:
...
4. And finally — after restarting EagleERP — navigating to the test
