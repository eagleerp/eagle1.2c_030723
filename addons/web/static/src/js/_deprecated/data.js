 ...
 * consumed by EagleERP's RPC APIs.
 *
...
var DataSet =  Class.extend(mixins.PropertiesMixin, {
    /**
     * Collection of EagleERP records, used to share records and the current selection between views.
     *
     * @constructs instance.web.DataSet
     *
     * @param {String} model the EagleERP model this dataset will manage
     */
    ...
    get_id_index: function (id) {
        for (var i=0, ii=this.ids.length; i<ii; i++) {
            ...
            // EagleERP ids fetched from the DOM as string. (eg: dhtmlxcalendar)
            // EagleERP ids can be non-numeric too ! (eg: recursive events in calendar)
            if (id == this.ids[i]) {
                return i;
            }
        }
     ...
     * @param {Array} [domain=[]] filters for the objects returned, EagleERP domain
     ...
     * an SQL-like form usable by EagleERP's ORM.
     *
...
var Model = Class.extend({
    ...
    @param {String} name name of the EagleERP model this object is bound to
    @param {Object} [context]
    @param {Array} [domain]
    */
    ...
    /**
     * Call a method (over RPC) on the bound EagleERP model.
     *
