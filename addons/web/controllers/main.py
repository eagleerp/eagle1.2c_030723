...
from eagle.modules import get_resource_path
from eagle.tools import crop_image, topological_sort, html_escape, pycompat
from eagle.tools.mimetypes import guess_mimetype
from eagle.tools.translate import _
from eagle.tools.misc import str2bool, xlwt, file_open
from eagle.tools.safe_eval import safe_eval
from eagle import http
from eagle.http import content_disposition, dispatch_rpc, request, \
        ...
        if not content:
            ...
            if not (width or height):
                ...
                if suffix in ('small', 'medium', 'big'):
                    content = getattr(eagle.tools, 'image_resize_image_%s' % suffix)(content)
        ...
        if crop and (width or height):
            ...
            if not upper_limit:
                ...
                if height > 500:
                    height = 500
            try:
                content = eagle.tools.image_resize_image(base64_source=content, size=(width or None, height or None),
...
class Export(http.Controller):
    ...
    def get_fields(self, model, prefix='', parent_name= '',
        ...
        fields_sequence = sorted(fields.items(),
            key=lambda field: (field[0] not in ['id', '.id', 'display_name', 'name'], eagle.tools.ustr(field[1].get('string', ''))))
