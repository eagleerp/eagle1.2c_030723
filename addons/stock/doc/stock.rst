...
A stock move is the elementary model in EagleERP that can move stock between 2 locations.  
...
You might have a weird feeling talking about moving from location A to location B, even for deliveries and incoming shipments.  That is because EagleERP uses a double-entry concept similar to double-entry accounting.  In EagleERP you do not talk of disappearance, consumption or loss of products: instead you speak only about stock moves from one place to another.
...
A warehouse corresponds also to a location.  As the locations are hierarchical, EagleERP will create one parent location for the warehouse that contains all the different locations in it.  
...
A product can be MTO or MTS.  When a product is handled MTO, it means we will handle each order (e.g. sale order) individually and procure what is necessary, separately for every order.  When a product is handled MTS, we wait until there are sufficient orders and then we order everything at once taking into account a minimum stock (or a stock forecast) into account.  In EagleERP, we can automate minimum stock rules through reordering rules (or orderpoints) as shown in the next chapter. 
...
In this example, EagleERP will propose placing two orders with the vendor of product WOOD002. Each of these orders can be for a different planned date. Before confirming these orders, the purchasing manager could group (merge) these orders into a single order.
...
When we want to configure this in EagleERP, we will typically start by configuring the warehouses.  The logic for choosing the routes in EagleERP, is to first check those of the warehouse and then those of the product and product categories.  So, the logic for configuring, is to put the generic routes on the warehouses and to put exceptions on these general rules on product and product categories.  
