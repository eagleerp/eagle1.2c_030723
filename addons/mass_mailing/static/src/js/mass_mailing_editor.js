eagle.define('mass_mailing.editor', function (require) {
"use strict";
...
var eagle_top = window.top.eagle;
...
snippets_editor.Class.include({
    _computeSnippetTemplates: function (html) {
        ...
        $mail_themes_upgrade.on("click", function (e) {
            ...
            e.preventDefault();
            eagle_top[window.callback+"_do_action"]("mass_mailing.action_mass_mailing_configuration");
        });
        ...
        $dropdown.on("click", ".dropdown-item", function (e) {
            ...
            // Notify form view
            eagle_top[window.callback+"_downup"]($editable_area.addClass("o_dirty").html());
        });
...
eagle_top[callback+"_updown"] = function (value, fields_values, field_name) {
    if (!window || window.closed) {
        delete eagle_top[callback+"_updown"];
        return;
    }
    ...
    if (fields_values.mailing_model && editor_enable) {
        if (value.indexOf('on_change_model_and_list') !== -1) {
            eagle_top[callback+"_downup"](_val);
        }
    }
};
