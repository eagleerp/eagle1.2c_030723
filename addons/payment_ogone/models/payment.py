...
from eagle.addons.payment_ogone.controllers.main import OgoneController
from eagle.addons.payment_ogone.data import ogone
    ...
    def ogone_form_generate_values(self, values):
        ...
        if self.save_token in ['ask', 'always']:
            temp_ogone_tx_values.update({
                'ALIAS': 'EAGLE-NEW-ALIAS-%s' % time.time(),    # something unique,
...
class PaymentTxOgone(models.Model):
    ...
    def ogone_s2s_do_transaction(self, **kwargs):
        ...
        reference = self.reference or "EAGLE-%s-%s" % (datetime.datetime.now().strftime('%y%m%d_%H%M%S'), self.partner_id.id)
    ...
    def _ogone_s2s_get_tx_status(self):
        account = self.acquirer_id
        #reference = tx.reference or "EAGLE-%s-%s" % (datetime.datetime.now().strftime('%Y%m%d_%H%M%S'), tx.partner_id.id)
...
class PaymentToken(models.Model):
    ...
    def ogone_create(self, values):
        if values.get('cc_number'):
            ...
            alias = 'EAGLE-NEW-ALIAS-%s' % time.time()
