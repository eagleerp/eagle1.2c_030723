...
from eagle.addons.test_mail.data.test_mail_data import \
...
class TestMailgateway(BaseFunctionalTest, MockEmails):
    ...
    @mute_logger('eagle.addons.mail.models.mail_thread', 'eagle.addons.mail.models.mail_mail', 'eagle.models.unlink')
    ...
    def test_message_process_model_res_id(self):
        """ Incoming email with ref holding model / res_id but that does not match any message in the thread: must raise since EagleERP saas-3 """
        ...
        # Eagle 10 update: compat mode has been removed and should not work anymore and does not depend from hostname
        self.assertRaises(ValueError,
                          self.format_and_process,
                          MAIL_TEMPLATE, email_from=formataddr((self.partner_1.name, self.partner_1.email)),
