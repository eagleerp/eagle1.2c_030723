...
import eagle
from eagle.tools.misc import DotDict
...
def MockRequest(
    ...
    with contextlib.ExitStack() as s:
        eagle.http._request_stack.push(request)
        s.callback(eagle.http._request_stack.pop)
        s.enter_context(patch('eagle.http.root.get_db_router', router))
