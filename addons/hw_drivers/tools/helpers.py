...
class IoTRestart(Thread):
    """
    Thread to restart eagle server in IoT Box when we must return a answer before
    """
        ...
        try:
            ...
            if response.status == 200:
                ...
                if db_branch != local_branch:
                    ...
                    subprocess.check_call(["rm", "-rf", "/home/pi/eagle/addons/hw_drivers/drivers/*"])
...
def check_image():
    ...
    """
    url = 'http://nightly.eagle-erp.com/master/posbox/iotbox/SHA1SUMS.txt'
    urllib3.disable_warnings()
    http = urllib3.PoolManager(cert_reqs='CERT_NONE')
...
def download_drivers(auto=True):
    ...
    if server:
        ...
        try:
            ...
            if resp.data:
                subprocess.check_call(["sudo", "mount", "-o", "remount,rw", "/"])
                drivers_path = Path.home() / 'eagle/addons/hw_drivers/drivers'
