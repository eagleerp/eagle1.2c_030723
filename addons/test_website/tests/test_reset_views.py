...
class TestWebsiteResetViews(eagle.tests.HttpCase):
    def do_test(self, name):
        self.browser_js(
            "/",
            "eagle.__DEBUG__.services['web_tour.tour'].run('%s')" % name,
            "eagle.__DEBUG__.services['web_tour.tour'].tours.%s.ready" % name,
            login="admin"
        )
    ...
    @mute_logger('eagle.addons.website.models.ir_http')
    ...
    # @mute_logger('eagle.addons.website.models.ir_http', 'eagle.addons.website.models.ir_ui_view')
