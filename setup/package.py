...
def _rpc_count_modules(addr='http://127.0.0.1', port=8069, dbname='mycompany'):
    ...
    if len(modules) > 1:
        ...
        if toinstallmodules:
            ...
            raise EagleTestError("Installation of package failed")
        else:
        ...
        raise EagleTestError("Package test: FAILED. Not able to install base.")
...
class DockerRpm(Docker):
    ...
    def gen_rpm_repo(self, args, rpm_filepath):
        ...
        self.run('createrepo /data/src/', temp_path, 'eagle-rpm-createrepo-%s' % TSTAMP)
