...
from eagle.tools import single_email_re
...
# Main classes
# ------------------------------------------------------------
class EagleSuite(unittest.suite.TestSuite):
...
class BaseCase(TreeCase, MetaCase('DummyCase', (object,), {})):
    """
    Subclass of TestCase for common EagleERP-specific code.
...
class ChromeBrowser():
    ...
    def _get_shotname(self, prefix, ext):
        ...
        base_file = os.path.splitext(eagle.tools.config['logfile'])[0]
        name = '%s_%s_%s.%s' % (base_file, prefix, timestamp, ext)
        return name

    def take_screenshot(self, prefix='failed'):
        if not eagle.tools.config['logfile']:
    ...
    def _save_screencast(self, prefix='failed'):
        ...
        if not eagle.tools.config['logfile']:
            ...
            return None
        sdir = tempfile.mkdtemp(suffix='_chrome_eagle_screencast')
        nb = 0
...
class HttpCase(TransactionCase):
    ...
    def _wait_remaining_requests(self):
        ...
        for thread in threading.enumerate():
            if thread.name.startswith('eagle.service.http.request.'):
                join_retry_count = 10
    ...
    def authenticate(self, user, password):
        ...
        # EagleERPSession.authenticate accesses the current request, which we
