...
from eagle.tools import cloc, config
from . import Command

class Cloc(Command):
    """\
    Eagle cloc is a tool to count the number of relevant lines written in
    ...
    It has two modes of operation, either by providing a path:
        ...
        eagle-bin cloc -p module_path

    Or by providing the name of a database:

        eagle-bin cloc --addons-path=dirs -d database
