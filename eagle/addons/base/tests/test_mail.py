...
from eagle.tools import html_sanitize, append_content_to_html, plaintext2html, email_split, misc, decode_smtp_header
...
class EmailConfigCase(SavepointCase):
    ...
    def test_email_from_rewrite(self):
        ...
        set_param('mail.catchall.domain', 'eagle.example.com')
        ...
        set_param('mail.force.smtp.from', False)
        set_param('mail.dynamic.smtp.from', 'notification@eagle.example.com')
        set_param('mail.catchall.domain', 'eagle.example.com')

        email_from, return_path = get_email_from('"Admin" <admin@test.example.com>')
        self.assertEqual(email_from, '"Admin" <notification@eagle.example.com>',
                         msg='Domain is not the same as the catchall domain, we should force the email FROM')
        self.assertEqual(return_path, 'notification@eagle.example.com')

        email_from, return_path = get_email_from('"Admin" <admin@eagle.example.com>')
        self.assertEqual(email_from, '"Admin" <admin@eagle.example.com>',
        ...
        set_param('mail.force.smtp.from', False)
        set_param('mail.dynamic.smtp.from', '"Your notification bot" <notification@eagle.example.com>')
        set_param('mail.catchall.domain', 'eagle.example.com')
