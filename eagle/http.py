...
# *broken*
NO_POSTMORTEM = (eagle.osv.orm.except_orm,
                 eagle.exceptions.AccessError,
                 eagle.exceptions.ValidationError,
                 eagle.exceptions.MissingError,
...
class WebRequest(object):
    ...
    @lazy_property
    def session(self):
        """ :class:`EagleERPSession` holding the HTTP session data for the
    ...
    * the handler method's result is returned as JSON-RPC ``result`` and
    ...
    def _handle_exception(self, exception):
        ...
        try:
            ...
            if not isinstance(exception, (eagle.exceptions.Warning, SessionExpiredException,
                                          eagle.exceptions.except_orm, werkzeug.exceptions.NotFound)):
...
HTTP methods). See
https://www.eagle-erp.com/documentation/12.0/developer/reference/http.html#csrf for
more details.
...
class EagleERPSession(werkzeug.contrib.sessions.Session):
    def __init__(self, *args, **kwargs):
        self.inited = False
        self.modified = False
        self.rotate = False
        super(EagleERPSession, self).__init__(*args, **kwargs)
        self.inited = True
    ...
    def _fix_lang(self, context):
        """ EagleERP provides languages which may not make sense and/or may not
...
class Root(object):
    """Root WSGI application for the EagleERP Web Client.
    """
    def __init__(self):
        self._loaded = False
    ...
    def session_store(self):
        ...
        return werkzeug.contrib.sessions.FilesystemSessionStore(
            path, session_class=EagleERPSession, renew_missing=True)

    @lazy_property
    def nodb_routing_map(self):
        _logger.info("Generating nondb routing")
        return routing_map([''] + eagle.conf.server_wide_modules, True)
    ...
    def load_addons(self):
        ...
        statics = {}
        for addons_path in eagle.modules.module.ad_paths:
