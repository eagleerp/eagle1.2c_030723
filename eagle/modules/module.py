...
class AddonsHook(object):
    """ Makes modules accessible through openerp.addons.* and eagle.addons.* """

    def find_module(self, name, path=None):
        if name.startswith(('eagle.addons.', 'openerp.addons.'))\
                and name.count('.') == 2:
            return self
    ...
    def load_module(self, name):
        ...
        openerp_name = re.sub(r'^eagle.addons.(\w+)$', r'openerp.addons.\g<1>', eagle_name)

        assert eagle_name not in sys.modules
        ...
        # create empty eagle.addons.* module, set name
        new_mod = types.ModuleType(eagle_name)
        ...
        setattr(eagle.addons, addon_name, new_mod)
...
def initialize_sys_path():
    """
    Setup an import-hook to be able to import EagleERP addons from the different
    addons paths.
    ...
    # add eagle.addons.__path__
    for ad in __import__('eagle.addons').addons.__path__:
...
def load_openerp_module(module_name):
    """ Load an EagleERP module, if not already loaded.
...
def get_test_modules(module):
    ...
    results = _get_tests_modules('eagle.addons', module)

    try:
        importlib.import_module('eagle.addons.base.maintenance.migrations.%s' % module)
    except ImportError:
        pass
    else:
        results += _get_tests_modules('eagle.addons.base.maintenance.migrations', module)
...
class TestStream(object):
    def __init__(self, logger_name='eagle.tests'):
...
def run_unit_tests(module_name, dbname, position='at_install'):
    ...
    from eagle.tests.common import TagsSelector, EagleSuite
    ...
    for m in mods:
        ...
        suite = EagleSuite(t for t in tests if position_tag.check(t) and config_tags.check(t))
