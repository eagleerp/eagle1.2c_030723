...
from eagle.tests.common import EagleSuite
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def __init__(self, host, port, app):
        # The EAGLE_MAX_HTTP_THREADS environment variable allows to limit the amount of concurrent
        ...
        self.max_http_threads = os.environ.get("EAGLE_MAX_HTTP_THREADS")
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        for path in eagle.modules.module.ad_paths:
...
class WorkerHTTP(Worker):
    ...
    def __init__(self, multi):
        ...
        # The EAGLE_HTTP_SOCKET_TIMEOUT environment variable allows to control socket timeout for
        ...
        sock_timeout = os.environ.get("EAGLE_HTTP_SOCKET_TIMEOUT")
        ...
        for mod in [m for m in get_modules() if '/%s/' % m in test_file]:
            for mod_mod in get_test_modules(mod):
                ...
                if test_path == mod_path:
                    tests = eagle.modules.module.unwrap_suite(
                        unittest.TestLoader().loadTestsFromModule(mod_mod))
                    suite = EagleSuite(tests)
...
def start(preload=None, stop=False):
    ...
    if eagle.evented:
        ...
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which Eagle is based upon in order to limit the memory usage for threaded workers.
