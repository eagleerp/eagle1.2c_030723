...
import eagle
from eagle.tools.config import config
...
class Cloc(object):
    ...
    def count_path(self, path, exclude=None):
        path = path.rstrip('/')
        exclude_list = []
        for i in eagle.modules.module.MANIFEST_NAMES:
    ...
    def count_modules(self, env):
        ...
        exclude_heuristic = [eagle.modules.get_module_path(m, display_warning=False) for m in STANDARD_MODULES]
        ...
        for module_name in module_list:
            module_path = os.path.realpath(eagle.modules.get_module_path(module_name))
            if module_path:
    ...
    def count_customization(self, env):
        ...
        # the module should be eagle/studio unless it comes from an imported module install
    ...
    def count_database(self, database):
        with eagle.api.Environment.manage():
            registry = eagle.registry(config['db_name'])
    ...
    def report(self, verbose=False, width=None):
        ...
        # Render
        s = fmt.format(k="Eagle cloc", lines="Line", other="Other", code="Code")
        s += hr
