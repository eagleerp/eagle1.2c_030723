...
class configmanager(object):
    def __init__(self, fname=None):
        ...
        self.options = {
            'admin_passwd': 'admin',
            'csv_internal_sep': ',',
            'publisher_warranty_url': 'http://services.eagle-erp.com/publisher-warranty/',
            'reportgz': False,
            'root_path': None,
        }
    ...
    def _parse_config(self, args=None):
        ...
        if os.name == 'nt':
            ...
            die(os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath),
                ...
                "them, preferrably '.eaglerc'.")

            if not os.path.isfile(rcfilepath) and os.path.isfile(old_rcfilepath):
                rcfilepath = old_rcfilepath

        self.rcfile = os.path.abspath(
            self.config_file or opt.config or os.environ.get('EAGLE_RC') or os.environ.get('OPENERP_SERVER') or rcfilepath)
        self.load()
